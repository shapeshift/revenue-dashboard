...
async function testJupiterJan31() {
  const startTimestamp = 1738281600 // Jan 31, 2026 00:00:00 UTC
  const endTimestamp = 1738367999 // Jan 31, 2026 23:59:59 UTC

  console.log('Testing Jupiter Integration for Jan 31, 2026')
  ...
  for (const token of TRACKED_TOKENS) {
    ...
    while (true) {
      ...
      for (const sig of signatures) {
        ...
        if (sig.blockTime < startTimestamp) {
          console.log(`Reached signature before start time: ${new Date(sig.blockTime * 1000).toISOString()}`)
          break
        }
        ...
        console.log(`Processing signature from ${new Date(sig.blockTime * 1000).toISOString()}: ${sig.signature}`)
    ...
    console.log(`\n${token.symbol} Summary for Jan 31, 2026:`)
    ...
    if (fees.length > 0) {
      ...
      console.log('\nFee details:')
      fees.forEach(f => {
        console.log(`  ${new Date(f.timestamp * 1000).toISOString()} - ${f.uiAmount} ${token.symbol}`)
        console.log(`    Tx: ${f.signature}`)
      })
    }
  }
}

testJupiterJan31().catch(console.error)
